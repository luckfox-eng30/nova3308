...
build_wifibt()
{
	...
	if [[ "$RK_WIFIBT_CHIP" =~ "ALL_AIC" ]];then
		echo "building AIC8800DC"
		$KMAKE M=$RKWIFIBT_DIR/drivers/aic8800dc modules
	fi
	...
	if [[ "$RK_WIFIBT_CHIP" = "AIC8800DC" ]];then
		$KMAKE M=$RKWIFIBT_DIR/drivers/aic8800dc modules
	fi
	...
	if [[ "$RK_WIFIBT_CHIP" = "AIC8800DC" ]];then
		echo "Copy AIC8800DC file to rootfs"
		if [ -d "$RKWIFIBT_DIR/firmware/aic8800dc" ]; then
			cp $RKWIFIBT_DIR/firmware/aic8800dc/* \
				$TARGET_DIR/lib/firmware/
			# cp $RKWIFIBT_DIR/drivers/$RK_WIFIBT_CHIP/* \
			# 	$TARGET_DIR/lib/modules/
		else
