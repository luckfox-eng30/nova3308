...
#else
#define VBUF_LENGTH             (17 * NBANDS)   /* for single-sized vbuf FIFO */
...
typedef struct _SubbandInfo
{
    ...
    short vindex;                           /* internal index for tracking position in vbuf */
} SubbandInfo;
...
void FDCT32HalfBuffer(int *buf, int *dest, int offset, int oddBlock, int gb);
