...
int Subband(MP3DecInfo *mp3DecInfo, short *pcmBuf)
{
    ...
    if (mp3DecInfo->nChans == 2)
    {
        ...
        for (b = 0; b < BLOCK_SIZE; b++)
        {
            FDCT32HalfBuffer(mi->outBuf[0][b], sbi->vbuf + 0 * 16, sbi->vindex, (b & 0x01), mi->gb[0]);
            FDCT32HalfBuffer(mi->outBuf[1][b], sbi->vbuf + 1 * 16, sbi->vindex, (b & 0x01), mi->gb[1]);
            PolyphaseStereoHalfBuffer(pcmBuf, sbi->vbuf + VBUF_LENGTH * (b & 0x01), polyCoef, sbi->vindex);
            sbi->vindex = (sbi->vindex - (b & 0x01)) & 7;
            pcmBuf += (2 * NBANDS);
        }
    }
    else
    {
        ...
        {
            FDCT32HalfBuffer(mi->outBuf[0][b], sbi->vbuf + 0 * 16, sbi->vindex, (b & 0x01), mi->gb[0]);
            PolyphaseMonoHalfBuffer(pcmBuf, sbi->vbuf + VBUF_LENGTH * (b & 0x01), polyCoef, sbi->vindex);
            sbi->vindex = (sbi->vindex - (b & 0x01)) & 7;
            pcmBuf += NBANDS;
        }
    }
