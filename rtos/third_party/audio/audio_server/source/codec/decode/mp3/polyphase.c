...
#ifndef SUBBAND_VBUS_HALFBUFFER
/**************************************************************************************
...
#else
#define MC0HM(x,index)  { \
    c1 = *coef;     coef++;     c2 = *coef;     coef++; \
    vLo = *(vb1+((x+index)&7));         vHi = *(vb1+(VB1_STEP-((x - index)&7))); \
    sum1L = MADD64(sum1L, vLo,  c1);    sum1L = MADD64(sum1L, vHi, -c2); \
}

#define MC1HM(x,index)  { \
    c1 = *coef;     coef++; \
    vLo = *(vb1+((x+index)&7)); \
    sum1L = MADD64(sum1L, vLo,  c1); \
}

#define MC2HM(x,index)  { \
        c1 = *coef;     coef++;     c2 = *coef;     coef++; \
        vLo = *(vb1+((x + index)&7));   vHi = *(vb1 +( VB1_STEP-((x - index)&7))); \
        sum1L = MADD64(sum1L, vLo,  c1);    sum2L = MADD64(sum2L, vLo,  c2); \
        sum1L = MADD64(sum1L, vHi, -c2);    sum2L = MADD64(sum2L, vHi,  c1); \
...
/**************************************************************************************
* Function:    PolyphaseMonoHalfBuffer
*
* Description: filter one subband and produce 32 output PCM samples for one channel,
               the function have the same effect as PolyphaseMono(),and only need
               half vbuf buffer
 *
*
* Inputs:      pointer to PCM output buffer
...
*                (note max filter gain - see polyCoef[] comments)
**************************************************************************************/
void PolyphaseMonoHalfBuffer(short *pcm, int *vbuf, const int *coefBase, int vindex)
{
    int i;
    const int *coef;
    int *vb1;
...
/***********Optimize malloc memory modified by cherry****************/
#define MC0HS(x,index)  { \
    c1 = *coef;     coef++;     c2 = *coef;     coef++; \
    vLo = *(vb1+((x+index)&7));     vHi = *(vb1+(VB1_STEP-((x-index)&7))); \
    sum1L = MADD64(sum1L, vLo,  c1);    sum1L = MADD64(sum1L, vHi, -c2); \
    vLo = *(vb1+VB1_RCH_STEP+((x+index)&7));    vHi = *(vb1+VB1_RCH_STEP+( VB1_STEP-((x-index)&7))); \
    sum1R = MADD64(sum1R, vLo,  c1);    sum1R = MADD64(sum1R, vHi, -c2); \
}


#define MC1HS(x,index)  { \
    c1 = *coef;     coef++; \
    vLo = *(vb1+((x+index)&7)); \
    sum1L = MADD64(sum1L, vLo,  c1); \
    vLo = *(vb1+VB1_RCH_STEP+((x+index)&7)); \
    sum1R = MADD64(sum1R, vLo,  c1); \
}

#define MC2HS(x,index)  { \
    c1 = *coef;     coef++;     c2 = *coef;     coef++; \
    vLo = *(vb1+((x+index)&7)); vHi = *(vb1+(VB1_STEP-((x-index)&7))); \
    sum1L = MADD64(sum1L, vLo,  c1);    sum2L = MADD64(sum2L, vLo,  c2); \
    sum1L = MADD64(sum1L, vHi, -c2);    sum2L = MADD64(sum2L, vHi,  c1); \
...
/**************************************************************************************
 * Function:    PolyphaseStereoHalfBuffer
 *
 * Description: filter one subband and produce 32 output PCM samples for each channel
                the function have the same effect as PolyphaseStereo(),and only need
                half vbuf buffer
 *
 * Inputs:      pointer to PCM output buffer
 *              number of "extra shifts" (vbuf format = Q(DQ_FRACBITS_OUT-2))
 ...
 * TODO:        add 32-bit version for platforms where 64-bit mul-acc is not supported
 **************************************************************************************/
void PolyphaseStereoHalfBuffer(short *pcm, int *vbuf, const int *coefBase, int vindex)
{
    int i;
    const int *coef;
    int *vb1;
