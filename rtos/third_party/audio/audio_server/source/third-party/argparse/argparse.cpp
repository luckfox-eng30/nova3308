 ...
 * in the LICENSE file.
 *
 * modified: martin.cheng@rock-chips.com
 *     date: 2020/5/3
...
int rt_argparse_init(struct argparse *self, struct argparse_option *options,
...
int rt_argparse_describe(struct argparse *self, const char *description, const char *epilog) {
    self->description = description;
    self->epilog      = epilog;
    return 0;
}

int rt_argparse_parse(struct argparse *self, int argc, const char **argv) {
    self->argc = argc - 1;
    self->argv = argv + 1;
    self->out  = argv;
...
int rt_argparse_usage(struct argparse *self) {
...
int rt_argparse_usage_string(struct argparse *self, char *usage) {
    const char *const *usages = self->usages;

    if (usages) {
        sprintf(usage, "%s Usage: %s\n", usage, *usages++);
        while (*usages && **usages)
            sprintf(usage, "%s   or: %s\n", usage, *usages++);
    } else {
        sprintf(usage, "%sUsage:\n", usage);
    }
    ...
    for (; options->type != ARGPARSE_OPT_END; options++) {
        ...
        pos = sprintf(usage, "%s    ", usage);
        if (options->short_name) {
            pos += sprintf(usage, "%s-%c", usage, options->short_name);
        }
        if (options->long_name && options->short_name) {
            pos += sprintf(usage, "%s, ", usage);
        }
        if (options->long_name) {
            pos += sprintf(usage, "%s--%s", usage, options->long_name);
        }
        if (options->type == ARGPARSE_OPT_INTEGER) {
            pos += sprintf(usage, "%s=<int>", usage);
        } else if (options->type == ARGPARSE_OPT_FLOAT) {
            pos += sprintf(usage, "%s=<flt>", usage);
        } else if (options->type == ARGPARSE_OPT_STRING) {
            pos += sprintf(usage, "%s=<str>", usage);
        }
        if (pos <= usage_opts_width) {
            pad = usage_opts_width - pos;
        } else {
...
int rt_argparse_help_cb(struct argparse *self, const struct argparse_option *option) {
    (void)option;
    rt_argparse_usage(self);
    exit(0);
}
